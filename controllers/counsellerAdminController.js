...
const updateCounseller = async (req, res) => {
   ...
   const {patient_id, staff_id, counselling_date, counselling_notes} = req.body;
   const patientTypes = await getPatientTypes();
   const staffTypes = await getStaffType();
   ...
   // check for duplicate Counseller name before inserting/updating
   ...
   try {
      if (id !== '') {
         ...
         // console.log('updatedPatient', updatedCounseller);
         if (updatedCounseller) {
            req.flash('success_msg', 'Counseller Data Successfully updated.');
         }
         return res.redirect('/counsellerAdmin/counsellerTableAdmin');
      }
   } catch (err) {
      logger.error(err);
   }
...
const editCounseller = async (req, res) => {
   const {id} = req.params;
   try {
      const rows = await Counseller.findByPk(id);
...
async function getCounsellerTable() {
   try {
      const rows = await db.query(`SELECT * FROM counseller_admin `, {
         type: QueryTypes.SELECT,
      });
...
module.exports = {counsellerRegister, counsellerTable, updateCounseller, deleteCounsellerData, editCounseller};
